...
/**
 * Deliveries — the week, by route day.
 *
 ...
 */
export default async function DeliveryWeekPage({ searchParams }: PageProps<"/ops/deliveries/week">) {
    ...
    <>
      <div className="page-head">
        ...
        <div className="actions">
          {regions.length > 1 ? (
            <div className="seg">
              <Link className={!region ? "on" : ""} href="/ops/deliveries/week">
                All regions
              </Link>
              {regions.map((r) => (
                <Link key={r} className={region === r ? "on" : ""} href={`/ops/deliveries/week?region=${r}`}>
                  {r}
                </Link>
              ))}
            </div>
          ) : null}
          <Link className="btn" href="/ops/documents">
            Print batch
          </Link>
          <Link className="btn primary" href="/ops/deliveries">
            Dispatch board
          </Link>
        </div>
      </div>
      ...
      <div className="spec" style={{ marginTop: 16 }}>
        <b>This is the plan, not the truck.</b> Scheduling an order books the day; building a route on the{" "}
        <Link href="/ops/deliveries">dispatch board</Link> decides which stops the driver runs, in what order, and
        mints the BOL numbers he carries. <b>Mark delivered</b> then writes the ledger and keg custody and starts the
        Net 30 clock — from delivery, not from order date.
      </div>
    </>
  );
}
